...
export const ReportPage = ({ dealCount, dealsTotal, dealsPayed, dealsPayedStefano, dealsTotalStefano }) => {
    ...
    const formattedDealsStefano = numeral(dealsPayedStefano / 100).format('0,0[.]00 $')
    const formattedDealsTotalStefano = numeral(dealsTotalStefano / 100).format('0,0[.]00 $')
    const dealsPending = numeral((dealsTotal - dealsPayed) / 100).format('0,0[.]00 $')
    const dealsPendingStefano = numeral((dealsTotalStefano - dealsPayedStefano) / 100).format('0,0[.]00 $')
    ...
    return (
        <div className="page-header">
            <div className="content-container">
                ...
                <h2 className="page-header__title">Stefano: <span>{formattedDealsTotalStefano}</span> - Pagate: <span>{formattedDealsStefano}</span> - Pendenti: <span>{dealsPendingStefano}</span></h2>
            </div>
        </div>
    )
}
