...
const ImmoscoutAPI = ({ oggetto, startEditOggetto }) => {
  const connectToIS24 = (base_url, method) => {
    ...
    const oauth_token = 'YOUR TOKEN HERE';
    ...
    const encodedTokenSecret = encodeURIComponent(
      'YOUR ENCODE URI COMPONENT HERE'
    );
    ...
    const oAuth = `OAuth oauth_consumer_key="YOUR CONSUMER KEY HERE",oauth_nonce="${oauth_nonce}",oauth_signature="${encoded_oauth_signature}",oauth_signature_method="HMAC-SHA1",oauth_timestamp="${oauth_timestamp}",oauth_token="${encoded_oauth_token}",oauth_version="1.0"`;
    return oAuth;
  };
