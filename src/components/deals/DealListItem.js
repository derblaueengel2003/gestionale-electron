...
const DealListItem = ({ 
    ...
    return (
        <div>
        <Link className={uid === 'pCu3H2GQfPWQxMNGwIVTc0Ag0fg1' ? `list-item ${payedStefano && 'list-item--paid'}` : `list-item ${payed && 'list-item--paid'}`} to={`/view/${id}`}>
            <div>
                ...
                {`To-dos completati: ${todoCount} di 11`}<ProgressBar percentage={(todoCount * 100) / 11} />
            </div>
            <div>
                {uid === 'pCu3H2GQfPWQxMNGwIVTc0Ag0fg1' ? <h3 className="list-item__data">{numeral(provvStefano / 100).format('0,0[.]00 $')}</h3> : <h3 className="list-item__data">{numeral(provvM2square / 100).format('0,0[.]00 $')}</h3>} 
            </div>
