...
export class ViewDealPage extends React.Component {
    render() {
        ...
        const acquirente = this.props.clienti.find((cliente) => cliente.id === acquirenteId)
                ...
                <div className="content-container">
                    ...
                    <div className="list-body">
                        <div className="list-item">
                            <div>
                                {prezzoDiVendita > 0 && <h3 className="list-item__title">Prezzo di Vendita: {numeral(prezzoDiVendita / 100).format('0,0[.]00 $')}</h3>}
                                {createdAt > 0 && <span className="list-item__sub-title">Data prenotazione: {moment(createdAt).format('DD MMMM, YYYY')}</span>}
                                {dataRogito > 0 && <h4 className="list-item__sub-title">Data rogito: {moment(dataRogito).format('DD MMMM, YYYY')}</h4>}
                                {note.length > 0 && <span className="list-item__sub-title">Note: {note}</span>}
                            </div>
                            <div>
                                {amount > 0 && <h3 className="list-item__title">Provvigione: {uid === 'JzFEsotsQwhMMAeJeWDM8Jv2qGb2' || uid === 'BNhRvZCcvMPr54unKlYSSliPel42' ? numeral(amount / 100).format('0,0[.]00 $') : ''}</h3>}
                                {consulenteVendita > 0 && <h4 className="list-item__sub-title">Cliente di: {consulenteVendita}</h4>}
                                {uid === 'JzFEsotsQwhMMAeJeWDM8Jv2qGb2' || uid === 'BNhRvZCcvMPr54unKlYSSliPel42' ? provvM2square > 0 && <h4 className={`list-item__sub-title ${this.props.fatture.payed && 'list-item--paid'}`}>m2Square: {numeral(provvM2square / 100).format('0,0[.]00 $')}</h4> : ''}
                                {provvStefano > 0 && <h4 className={`list-item__sub-title ${payedStefano && 'list-item--paid'}`}>Stefano: {numeral(provvStefano / 100).format('0,0[.]00 $')}</h4>}
                                {payedAtStefano > 0 && <span className="list-item__sub-title">Pagata a Stefano il: {moment(payedAtStefano).format('DD MMMM, YYYY')}</span>}
                                {uid === 'JzFEsotsQwhMMAeJeWDM8Jv2qGb2' || uid === 'BNhRvZCcvMPr54unKlYSSliPel42' ? provvAgenziaPartner > 0 && <h4 className={`list-item__sub-title ${payedAgenziaPartner && 'list-item--paid'}`}>Provvigione Agenzia Partner: {numeral(provvAgenziaPartner / 100).format('0,0[.]00 $')}</h4> : ''}
                            </div>
                        </div>
                    </div>
                </div>
                <OggettiList oggetto={oggetto} />
                {venditoreId.length > 0 && <div><ClientiList cliente={venditore} ruolo={'Verkäufer'} /></div>}
                {venditoreId2.length > 0 && <div><ClientiList cliente={venditore2} ruolo={'2. Verkäufer'} /></div>}
                {acquirenteId.length > 0 && <div><ClientiList cliente={acquirente} ruolo={'Käufer'} /></div>}
                {acquirenteId2.length > 0 && <div><ClientiList cliente={acquirente2} ruolo={'2. Käufer'} /></div>}
                {agenziaPartnerId.length > 0 && <div><ClientiList cliente={agenziaPartner} ruolo={'Agenzia Partner'} /></div>}                 
                <div className="content-container">
                    {uid === 'JzFEsotsQwhMMAeJeWDM8Jv2qGb2' && <Link className="button button--secondary" to={`/edit/${id}`}>Modifica Provvigione</Link>}
